...
async def set_bot_commands(dp: Dispatcher):
    """Устанавливает команды бота для пользовательского интерфейса бота."""
...
async def ssh_connection(state: FSMContext, command: str = None, response: bool = False) -> [None, str]:
    """Подключается по SSH и немедленно отключается, при необходимости выполняет команду и возвращает ее результат."""
